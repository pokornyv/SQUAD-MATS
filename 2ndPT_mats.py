...
elif p.P['param'] == 'h':   par = h
elif p.P['param'] == 'P':   par = P
else:                       par = U	## default

WriteAux = p.P['WriteAux']    ## write auxiliary files?
...
.format(int(NMatsP),float(OmegaN(NMatsP,beta))),logfname)
...
PrintAndWrite('{0: .5f}\t{1: .5f}\t{2: .5f}\t{3: .5f}\t{4: .8f}\t{5: .8f}\t{6: .8f}\t:HF_OUT'\
...
PrintAndWrite('{0: .5f}\t{1: .5f}\t{2: .5f}\t{3: .5f}\t{4: .8f}\t{5: .8f}\t{6: .8f}\t:2nd_zero_OUT'\
.format(U,eps,h,P,n,m,mu),logfname)
...
PrintAndWrite('{0: .5f}\t{1: .5f}\t{2: .5f}\t{3: .5f}\t{4: .8f}\t{5: .8f}\t{6: .8f}\t:2nd_OUT'\
.format(U,eps,h,P,n,m,mu),logfname)

## write HDF5 archive
if WriteHDF5:
	PrintAndWrite('  Output saved to '+hdf5file+' archive.',logfname)
